...
let inputArr = input.split(',').map(num => parseInt(num, 10));

let position = 0;

const read = pos => {
  const opcode = inputArr[pos];
  let num1 = inputArr[inputArr[pos + 1]];
  let num2 = inputArr[inputArr[pos + 2]];
  let storePos = inputArr[pos + 3];
  let res;
  ...
  inputArr[storePos] = res;
};

inputArr[1] = 12;
inputArr[2] = 2;

while (inputArr[position] !== 99) {
  read(position);
  position += 4;
}
